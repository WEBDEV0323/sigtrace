...
class Tracker
{
    ...
    */
    ...
    public function getUserType($groupId)
    {
        ...
        $query = "SELECT super_admin FROM role WHERE rid = ".$groupId;
        $statements = $this->_adapter->query($query);
        $results = $statements->execute();
        $resultSet = new ResultSet;
        $resultSet->initialize($results);
    ...
    public function updateUserSession($uId, $trackerId, $roleId)
    {
        try {
            $query = "SELECT count(u_id) FROM user_role_session WHERE u_id = ".$uId." AND tracker_id=".$trackerId;
            $statements = $this->_adapter->query($query);
            $results = $statements->execute();
            $resultSet = new ResultSet;
            $resultSet->initialize($results);
            ...
            $count = $resultSet->count();
            if ($count > 0) {
                $statements = $this->_adapter->query("DELETE FROM user_role_session WHERE u_id = ".$uId." AND tracker_id=".$trackerId);
                $statements->execute();
            }
            $statements1 = $this->_adapter->query("INSERT INTO user_role_session SET u_id = ".$uId.", tracker_id=".$trackerId.", role_id=".$roleId);
            $statements1->execute();
        } catch(\Exception $e) {
