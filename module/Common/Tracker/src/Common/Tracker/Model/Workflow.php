...
class Workflow extends AbstractActionController
{
    ...
    public function takeBackup($table) 
    {
        ...
        ob_start();
        $backup_file = dirname(dirname(dirname(dirname(dirname(dirname(dirname(__DIR__)))))))."/public/backup/deletedWorkflowBackup/".$table."_".date("Y-m-d_H-i-s").'_history.sql';
        if (!file_exists(dirname(dirname(dirname(dirname(dirname(dirname(dirname(__DIR__)))))))."/public/backup/deletedWorkflowBackup")) {
            mkdir(dirname(dirname(dirname(dirname(dirname(dirname(dirname(__DIR__)))))))."/public/backup/deletedWorkflowBackup", 0777, true);
        }
        chmod(dirname(dirname(dirname(dirname(dirname(dirname(dirname(__DIR__)))))))."/public/backup/deletedWorkflowBackup", 0777);
    ...
    */
    ...
    public function saveFields($post)
    {
        ...
        $namesArr = isset($post['label_names']) ? filter_var_array($post['label_names'], FILTER_SANITIZE_STRING) : array();
        $types = isset($post['types']) ? filter_var_array($post['types'], FILTER_SANITIZE_STRING) : array();
        $formulas = isset($post['formula_id']) ? filter_var_array($post['formula_id'], FILTER_SANITIZE_STRING) : array();
        $expected = isset($post['expected']) ? filter_var_array($post['expected'], FILTER_SANITIZE_STRING) : array();
        $kpivalues = isset($post['kpivalues']) ? filter_var_array($post['kpivalues'], FILTER_SANITIZE_STRING) : array();
        $fieldSortOrder = isset($post['field_sort_order'])?filter_var_array($post['field_sort_order'], FILTER_SANITIZE_STRING) :array();
        $fieldIdArr = isset($post['field_id_arr']) ? filter_var_array($post['field_id_arr'], FILTER_SANITIZE_STRING) : array();
        $defaultvalues = isset($post['defaultvalues']) ? filter_var_array($post['defaultvalues'], FILTER_SANITIZE_STRING) : array();
        $formulaArray = array();
        $tableArr = array();
                ...
                if (!empty($fieldIdArr)) {
                    ...
                    foreach ($labelArr as $key => $value) {
                        ...
                        if ($count == 0) {
                            ...
                            $sql = new Sql($this->_adapter);
                            $newFieldsData[] = "{Order: ".$sortOrder.", 'Field Label Name': '".$label."','Type':'".$fieldType."','KPI':'".$kpi."','Default Value':'".$default."'}";
                            $newData = array(
                    ...
                    foreach ($namesArr as $key => $fieldName) {
                        ...
                        if ($count == 0) {
                            ...
                            $newFieldsData[] = "{Order: ".$sortOrder.", 'Field Label Name': '".$label."','Field Name':'".$fieldName."','Type':'".$fieldType."','KPI':'".$kpi."','Default Value':'".$default."','Options':'".$options."'}";
                            $insert->values($newData);
                            $statement = $sql->prepareStatementForSqlObject($insert);
                            $results = $statement->execute();
