<?php
namespace SigTRACE\Quantitative\Model;

use Zend\Mvc\Controller\AbstractActionController;
class Quantitative extends AbstractActionController
{
    protected $adapter;
    ...
    public function getQuantitativeAnalysis($productId, $trackerId, $formId, $configValue, $dashboardId, $filterArr)
    {    
        ...
        $query = "SELECT qa.pt_id, qa.soc_name, qa.rank, qa.preferred_term, qa.mc_name, qa.medical_evaluation, IFNULL(qa.priority, '-') as priority, qa.ror_all, qa.rationale,
        ...
        foreach ($result as $key => $val) {
            ...
            $currentSerious = "cond=".base64_encode("ptname='".$val['preferred_term']."' and (seriousnessevent = 'serious') and $filterArr");
            $currentNotSerious = "cond=".base64_encode("ptname='".$val['preferred_term']."' and (seriousnessevent = 'not serious') and $filterArr");
            $currentTotal = "cond=".base64_encode("ptname='".$val['preferred_term']."' and $filterArr");

            $cumulativeSerious = "cond=".base64_encode("ptname='".$val['preferred_term']."' and (seriousnessevent = 'serious') ");
            $cumulativeNotSerious = "cond=".base64_encode("ptname='".$val['preferred_term']."' and (seriousnessevent = 'not serious') ");
            $cumulativeTotal = "cond=".base64_encode("ptname='".$val['preferred_term']."' ");
            
            $result[$key]['current_serious_frequency'] = '<a href="/dashboard/list/'.$trackerId.'/'.$formId.'/'.$dashboardId.'/'.$productId.'?'.$currentSerious.'"> '.$val['current_serious_frequency'].' </a>';
            $result[$key]['current_nonserious_frequency'] = '<a href="/dashboard/list/'.$trackerId.'/'.$formId.'/'.$dashboardId.'/'.$productId.'?'.$currentNotSerious.'"> '.$val['current_nonserious_frequency'].' </a>';
            $result[$key]['current_total'] = '<a href="/dashboard/list/'.$trackerId.'/'.$formId.'/'.$dashboardId.'/'.$productId.'?'.$currentTotal.'"> '.$val['current_total'].' </a>';

            $result[$key]['cumulative_serious_frequency'] = '<a href="/dashboard/list/'.$trackerId.'/'.$formId.'/'.$dashboardId.'/'.$productId.'?'.$cumulativeSerious.'"> '.$val['cumulative_serious_frequency'].' </a>';
            $result[$key]['cumulative_non_serious_frequency'] = '<a href="/dashboard/list/'.$trackerId.'/'.$formId.'/'.$dashboardId.'/'.$productId.'?'.$cumulativeNotSerious.'"> '.$val['cumulative_non_serious_frequency'].' </a>';
            $result[$key]['cumulative_total'] = '<a href="/dashboard/list/'.$trackerId.'/'.$formId.'/'.$dashboardId.'/'.$productId.'?'.$cumulativeTotal.'"> '.$val['cumulative_total'].' </a>';
        }
