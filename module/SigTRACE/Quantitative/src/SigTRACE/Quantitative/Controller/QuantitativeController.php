<?php
namespace SigTRACE\Quantitative\Controller;
...
class QuantitativeController extends AbstractActionController
{
    ...
    public function getService()
    {
        if (!$this->_serviceMapper) {
            $sm = $this->getServiceLocator();
            $this->_serviceMapper = $sm->get('Quantitative\Model\Quantitative');
        }
    ...
    public function getDbService()
    {
        if (!$this->_dbMapper) {
            $sm = $this->getServiceLocator();
            $this->_dbMapper = $sm->get('Quantitative\Model\Db');
        }
    ...
    public function viewicsrAction()
    {
        set_time_limit(0);
        $session = new SessionContainer();
        $userContainer = $session->getSession("user");        
    ...
    public function updateQuantitativeAction()
    {
        set_time_limit(0);
        ...
        foreach ($products as $product) {
            $this->getService()->updateQuantitativeAnalysis(109, 199, $product['product_id']);
        }
        exit;
    }
    ...
    public function getDataAction()
    {
        ...
        if (!isset($userContainer->u_id)) {
            ...
            $activeSubstance = $this->getService()->getActiveIngradientNameById($productId);
            $res = $this->getService()->getQuantitativeAnalysis($productId, $trackerId, $formId, $configValue, $dashboardId, $allFilter);
            
            $allData['data'] = $res;
            $allData['activeSubstance'] = $activeSubstance;
            $allData['startDate'] = Date('j-M-Y', strtotime($startDate));
            $allData['endDate'] = Date('j-M-Y', strtotime($endDate));
            $allData['startDateOrig'] = $startDate;
            $allData['endDateOrig'] = $endDate;
