...
class IndexController extends AbstractActionController
{
    ...
    public function getCasedataHelperService() 
    {
        if (!$this->_adminMapper) {
            $sm = $this->getServiceLocator();
            $this->_adminMapper = $sm->get('SignalCalendar\Helper\SignalCalendarHelper');
        }
    ...
    public function getCasedataService() 
    {
        if (!$this->_casedataMapper) {
            $sm = $this->getServiceLocator();
            $this->_casedataMapper = $sm->get('SignalCalendar\Model\Casedata');
        }
    ...
    {
        //print_r ("Import section line# 80"); die;
        set_time_limit(0);
        ...
        if (!isset($userContainer->u_id)) {
            ...
            if ($trackerId != 0 && in_array($trackerId, $trackerIds)) {
                ...
                $configValue = $this->getCasedataService()->getsignalcalendardata($trackerId);
                // $dashboardId = '10';
    ...
    {
        //print_r ("indexcontroller line# 104"); die;
        set_time_limit(0);
        $session = new SessionContainer();
        $userContainer = $session->getSession("user");
        $trackerContainer = $session->getSession('tracker');
       ...
       //print_r ($configValue); die;
        $view = new ViewModel();
        $view->setVariables(array('trackerId' => $trackerId, 'formId' => $formId, 'configValue' => $configValue));
        return $view;
    }
                ...
                if ($fileSize <= $allowedFileSize) {
                    if (count($tempArray) > 0) {
                        ...
                        foreach ($tempArray as $arrInput) {
                            ...
                            $InserCsvData = $this->getCasedataService()->insertlitimportdata($tem, $userdetails);
                        }
                    }
                    if ($tempFileName != '') {
                        $fileName = $post['file']['name'];
    ...
    public function signalcalendarImportAction()
    {
        ...
        if ($request->isPost()) {
           ...
           // $configwherecheck = $uploadedfiletype . '_'. 'settings';
            ...
            //   print_r ($signalcal); die;
            //   print_r ("filesize". $fileSize);
            //   print_r ("<br>");
            //   print_r ("filesize". $allowedFileSize); die;
                ...
                if ($fileSize <= $allowedFileSize) {
                    if (count($signalcal) > 0) {
                        //$userdetails = $userContainer->u_id;
                            ...
                            //$tem = array_change_key_case($arrInput, CASE_LOWER);
                            //  print_r ("tracker id1: ".$trackerId); die;
                            $InserCsvData = $this->getCasedataService()->insertsignalcalendardata($signalcal, $tableName, $changedFileName);
                    ...
                    if ($tempFileName != '') {
                            ...
                            $fileInfo = pathinfo($fileName);
                            $newFileName = $fileInfo['filename'] . "_" . $hostName . "_" . Date('YmdHis') . "." . $fileInfo['extension'];
                            $keyname = $child_folder . "/" . $newFileName;
                        if (file_exists($post['file']['tmp_name'])) {
                            $awsResult = $this->forward()->dispatch('Common\FileHandling\Controller\FileHandling', array('action' => 'uploadFilesToAws', 'keyname' => $keyname, 'filepath' => $post['file']['tmp_name'], 'del' => '1'));
                        }
                        $child_folder = str_replace('_', ' ', $child_folder);
