...
class SignalCalendar extends AbstractActionController
{
    ...
    public function getArchiveFilters($trackerId, $formId, $typeOfFilter = '')
    {
        $arr = array();
        try {
    ...
    public function getArchiveQueries($trackerId, $formId, $typeOfReport = '')
    {
        $arr = array();
        try {
            $sql = new Sql($this->_adapter);
    ...
    public function getArchiveCountReports($trackerId, $formId)
    {
        $sql = new Sql($this->_adapter);
        $select = $sql->select();
        $select->columns(array('filter_name', 'filter_label', 'condition', 'countReportQuery'));
        $select->from('archive_filters');
    ...
    public function getArchiveReports($archiveQueries, $trackerId, $formId, $countReportQueries = array(), $filter="")
    {
        ...
        $types = array();
        $ids = array();
        $filterQueryArray = $this->getArchiveFilters($trackerId, $formId, $filter);
        ...
        foreach ($archiveQueries as $archive) {
            $table = $archive['countQuery'];
            $where = ($archive['where'] != '')?" WHERE ".$archive['where']:"";
            $types[$archive['typeOfReport']] = $archive['label'];
            $ids[$archive['typeOfReport']] = $archive['archive_id'];
            $query .= ",(".$table." ".$where." ".$filterCondition.") as '".$archive['typeOfReport']."'";
        } 
        if (!empty($countReportQueries)) {
            foreach ($countReportQueries as $countReport) {
                // $countQuery = " WHERE 1 ";
                $countQuery .= ($countReport['condition'] != '' && $countReport['condition'] != null)?" AND ".$countReport['condition']:" ";
        ...
        if ($count > 0) {
            ...
            foreach ($arr[0] as $k=>$v) {
                $result[] = array("archive_id"=>$ids[$k], "name"=>$types[$k], "type"=>$k, "count"=>$v);
            }
        }
        return $result;
    }
    ...
    // {
    //     $listsQueryData = $this->getArchiveQueries($trackerId, $formId, $type);
    //     $filterQueryData = $this->getArchiveFilters($trackerId, $formId, $filter);
    //     $arr = array();
    //     try{
    //         if (!empty($listsQueryData)) {
    //             foreach ($listsQueryData as $key=>$list) {
    //                 $dbQuery = ($list['listQuery'] != '')?$list['listQuery']:"";
    //                 $where = "WHERE is_deleted = 'Yes'";//($list['where'] != '')?" WHERE ".$list['where']:"";
    ...
    //             } 
    //             $statements = $this->_adapter->createStatement($query);print_r($statements);exit;
    //             $results = $statements->execute();
    //             $resultSet = new ResultSet;
    //             $resultSet->initialize($results);
    //             $resultSet->buffer();
    ...
    public function fetchAllData($trackerId, $formId, $type, $filter)
    {
        ...
        $arr = array();
        //print_r ($listsQueryData); die;
        try{
            if (!empty($listsQueryData)) {
                foreach ($listsQueryData as $key=>$list) {
                    $dbQuery = ($list['listQuery'] != '')?$list['listQuery']:"";
                    $where =" where is_deleted = 'No'";// ($list['where'] != '')?" WHERE ".$list['where']:"";
                    $filter = ($filterQueryData[$key]['condition'] != '')?($where != '')?" AND ".$filterQueryData[$key]['condition']:" WHERE ".$filterQueryData[$key]['condition']:"";
                    $groupBy = isset($list["groupBy"])?" GROUP BY ".$list["groupBy"]:"";
                    $orderBy = isset($list["orderBy"])?" ORDER BY ".$list["orderBy"]:"";
                    $query = $dbQuery." ".$where." ".$filter." ".$groupBy." ".$orderBy;
    ...
    public function fetchDashboardData($fid, $tid)
    {      
        $query="SELECT dashboard_id FROM dashboard WHERE formId = $fid AND trackerId=$tid";
        $statements = $this->_adapter->createStatement($query);
        $statements->prepare();
        $results = $statements->execute();
        $arr = array();
    ...
    public function getDashboardQueries($trackerId, $formId, $typeOfReport = '')
    {
        ...
        try {
            ...
            $select->from('dashboard');
            if ($typeOfReport != '') {
                $select->where(array('trackerId' => $trackerId, 'formId'=>$formId, 'archived'=>0, 'dashboard_name '=>$typeOfReport)); 
            } else {
                $select->where(array('trackerId' => $trackerId, 'formId'=>$formId, 'archived'=>0));
            }
            $selectString = $sql->prepareStatementForSqlObject($select);
