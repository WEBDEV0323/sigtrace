...
class Field extends AbstractActionController
{
    ...
    public function getRolesForTracker($trackerId)
    {
        ...
        $select = $sql->select();
        $select->from('group');
        $select->where(array('tracker_id' => $trackerId, 'group_archived' => 0));
    ...
    public function takeBackup($tables) 
    {
        ...
        ob_start();
        $backup_file = dirname(dirname(dirname(dirname(dirname(__DIR__)))))."/public/backup/deletedFieldBackup/fields_".date("Y-m-d_H-i-s").'_history.sql';
        if (!file_exists(dirname(dirname(dirname(dirname(dirname(__DIR__)))))."/public/backup/deletedFieldBackup")) {
            mkdir(dirname(dirname(dirname(dirname(dirname(__DIR__)))))."/public/backup/deletedFieldBackup", 0777, true);
        }
        chmod(dirname(dirname(dirname(dirname(dirname(__DIR__)))))."/public/backup/deletedFieldBackup", 0777);
