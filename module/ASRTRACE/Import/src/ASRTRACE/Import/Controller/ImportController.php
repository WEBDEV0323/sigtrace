    ...
    public function getFileFromClientAction()
    {
        ...
        if ($request->isPost()) { 
                    ...
                    foreach ($dataArray as $row) {
                        foreach ($fieldDataTypeMapper as $cols) {
                            if (isset($row[$cols])) {
                                ...
                                if (($row[$cols]!= '') && !(Date('j-M-Y', strtotime($row[$cols])) == $row[$cols] || Date('d-M-Y', strtotime($row[$cols])) == $row[$cols])) {
                                    $message="Accepted Date Format is DD-MMM-YYYY";
                                    $flag=1;
                                    break;                                
                                } else {           
                                    $dataArray[$i][$cols]=Date('Y-m-d', strtotime($row[$cols]));                   
                                }
                            }
                        }
                        $i++;
            ...
            try{
                ...
                //                } else {
                //                    $message="Incompatiable file or Blank file";
                //                    $flag=1;
                //                }
            }catch(Exception $e) {
                echo $e->getMessage();
