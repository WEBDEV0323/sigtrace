...
class Email
{
    ...
    public function getAlltemplate($trackerId)
    {
        ...
        $select->join('form', 'notification_template.notification_template_form_id = form.form_id');
        $select->join('user', 'user.u_id = notification_template.notification_template_updated_by');
    ...
    public function saveTemplate($data, $uId)
    {
        ...
        if ($data['template_id'] > 0) {
            $qry='select if ((SELECT count("notification_template_id") FROM notification_template where notification_template_name="'.$data['template_name'].'" AND notification_template_id <>'.$data['template_id'].' AND notification_template_form_id ='.$data['form_id'].')>0, 3, 0) as `checkduplicate`';
        } else {
            $qry='select if ((SELECT count("notification_template_id") FROM notification_template where notification_template_name="'.$data['template_name'].'" AND notification_template_form_id ='.$data['form_id'].')>0, 3, 0) as `checkduplicate`';
        }
    ...
    */
    ...
    {
        // $this->db->delete("delete from `condition_for_templates` where condition_id=?", array($id));
