...
function ajaxCallAfterPageLoad(tracker_id,form_id,workflowId,record_id,relatedTarget) {
   ...
   if (typeof workflowId === 'undefined') {
      ...
      $.ajax({
         url:'/wp/getFieldsByWorkflowId/'+tracker_id+'/'+form_id+'/'+dashboardId+'/'+asId+'/'+record_id+'/'+workflowId, 
 ...
 function renderCallbackFields(response) {
   ...
   for(var i in response) {
      if (response[i]['can_insert']=='Yes' && recordId == 0) {
         ...
         switch (response[i]['field_type']) {
            case 'Text': 
               ...
               if(value == null) {value = '';}
               var textField=textFields(response[i]['fieldlabel'],response[i]['field_name'],'text',value,"",validation,response[i]['hide_field']);
               $("#fieldsDiv"+workflowId).append(textField);
               break;
            case 'Integer':
            case 'ReadOnly Integer':
               var integerField=integerFields(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],"",validation,response[i]['hide_field']);
            ...
            case 'ReadOnly Date':
               ...
               var dateField=dateTypeFields(response[i]['fieldlabel'],response[i]['field_name'],fieldType,value,dateFormat,validation,response[i]['hide_field']);
            ...
            case 'ReadOnly Date Time':
               ...
               var dateTimeField=dateTimeTypeFields(response[i]['fieldlabel'],response[i]['field_name'],fieldType,value,dateTimeFormat,validation,response[i]['hide_field']);
            ...
            case 'ReadOnly':
               ...
               var isValid=value.isValid();
                if(isValid) {
                    ...
                    var readOnlyField=readOnlyFields(response[i]['fieldlabel'],response[i]['field_name'],value,response[i]['hide_field'],record_id,validation);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);
                } else {
                    var readOnlyField=readOnlyFields(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],response[i]['hide_field'],record_id,validation);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);                   
                }
               break;
            case 'Text Area':
            case 'ReadOnly Text Area':
               var value = response[i]['fieldValue'];
               if(value == null) {value = '';}
               var textAreaField=textAreaFields(response[i]['fieldlabel'],response[i]['field_name'],value,'',validation,response[i]['hide_field']);
               $("#fieldsDiv"+workflowId).append(textAreaField);
               break;
            case 'Combo Box':
               var options=response[i]['options'];
               var comboBoxField=dropDownSingleSelectFields(response[i]['fieldlabel'],response[i]['field_name'],options,response[i]['fieldValue'],"",validation,response[i]['hide_field']);
               $("#fieldsDiv"+workflowId).append(comboBoxField);
               break;
            case 'User Role':
               if(response[i]['formula'] == 'CurrentUser'){
                  var currentUserField=currentUserFields(response[i]['fieldlabel'],response[i]['field_name'],currentUser,response[i]['hide_field']);
                  $("#fieldsDiv"+workflowId).append(currentUserField);
               } else {
                  var options=response[i]['options'];
                  var userRoleField=userRoleFields(response[i]['fieldlabel'],response[i]['field_name'],options,response[i]['fieldValue'],"",validation,response[i]['hide_field']);
            ...
            case 'Check Box': 
               ...
               var checkBoxField=checkBoxFields(response[i]['fieldlabel'],response[i]['field_name'],checkBoxOptions,response[i]['fieldValue'],commentvalue,"",validation,response[i]['hide_field']);
            ...
            case 'Formula': 
               ...
               var formulaField=formulaFields(response[i]['fieldlabel'],response[i]['field_name'],'text',value,formula,formula_dependent,response[i]['hide_field']);
            ...
            case 'Formula Combo Box': 
               ...
               var formula=escape(response[i]['formula']);
               var formula_dependent=response[i]['formula_dependent'];
               var formulaeDropDownField=formulaeDropDownFields(response[i]['fieldlabel'],response[i]['field_name'],options,response[i]['fieldValue'],formula,formula_dependent,response[i]['hide_field']);
            ...
            case 'Formula Date': 
               ...
               var formula=escape(response[i]['formula']);
               var formula_dependent=response[i]['formula_dependent'];
               var formulaDateField=formulaDateFields(response[i]['fieldlabel'],response[i]['field_name'],'text',response[i]['fieldValue'],formula,formula_dependent,dateFormat,response[i]['hide_field']);
               $("#fieldsDiv"+workflowId).append(formulaDateField);
               break;
            case 'File': 
               var fileField=fileFields(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],validation,response[i]['hide_field']);
               $("#fieldsDiv"+workflowId).append(fileField);
               break;
            case 'Dependent Text': 
               var formula=escape(response[i]['formula']);
               var formula_dependent=response[i]['formula_dependent'];
               var dependentTextField=dependentTextFields(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],formula,formula_dependent,response[i]['hide_field']);
            ...
            case 'Multivalue':
               ...
               var isValid=value.isValid();
                if(isValid) {
                    value=moment(value).format('DD-MMM-YYYY');
                    var readOnlyField=MultivalueFields(response[i]['fieldlabel'],response[i]['field_name'],value,response[i]['hide_field'], response[i]['multiple_values'],record_id,validation);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);
                } else {
                    var readOnlyField=MultivalueFields(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],response[i]['hide_field'], response[i]['multiple_values'],record_id,validation);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);                   
                }
               break;
            default:
      ...
      } else if (response[i]['can_update']=='Yes' && recordId != 0 ) {
         var validation = response[i]['validation_required'];
         if (validation == 1){validation = "required";}
         showSaveButton = 1;
         switch (response[i]['field_type']) {
            ...
            case 'ReadOnly Date Time':
               ...
               var isValid=value.isValid();
                if(isValid) {
                    value=moment(value).format('DD-MMM-YYYY');
                    var readOnlyField=readOnlyFields(response[i]['fieldlabel'],response[i]['field_name'],value,response[i]['hide_field'],record_id);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);
                } else {
                    var readOnlyField=readOnlyFields(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],response[i]['hide_field'],record_id);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);                   
                }
               break;
            case 'Text Area':
            ...
            case 'Multivalue':
               ...
               var isValid=value.isValid();
                if(isValid) {
                    value=moment(value).format('DD-MMM-YYYY');
                    var readOnlyField=MultivalueFields(response[i]['fieldlabel'],response[i]['field_name'],value,response[i]['hide_field'], response[i]['multiple_values'],record_id);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);
                } else {
                    var readOnlyField=MultivalueFields(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],response[i]['hide_field'], response[i]['multiple_values'],record_id);
                    $("#fieldsDiv"+workflowId).append(readOnlyField);                   
                }
               break;
            default:
         ...
         switch (response[i]['field_type']) {
            ...
            case 'Heading':
                  ...
                  break;
            case 'File': 
                  var fileField=fileFieldsDownload(response[i]['fieldlabel'],response[i]['field_name'],response[i]['fieldValue'],response[i]['hide_field'],record_id);
                  $("#fieldsDiv"+workflowId).append(fileField);
                  break;
            case 'Date': 
                  var dateFormat = response[i]['dateFormat'];
                  ...
                  var value = formatDateDDMMMYYYY(response[i]['fieldValue']);
                  if(value == 'Invalid date' || value == '0000-00-00') {value = '';}
                  var dateField=readOnlyFields(response[i]['fieldlabel'],response[i]['field_name'],value,response[i]['hide_field'],record_id,validation);
                  $("#fieldsDiv"+workflowId).append(dateField);
                  break;
            case 'Date Time': 
                  var dateTimeFormat=response[i]['dateTimeFormat'];  
                  ...
                  var value= moment(response[i]['fieldValue'],'YYYY-MM-DD HH:mm:ss').format('DD-MMM-YYYY hh:mm A');
                  if(value == 'Invalid date') {value = '';}
                  var dateTimeField=readOnlyFields(response[i]['fieldlabel'],response[i]['field_name'],value,response[i]['hide_field'],validation);
                  $("#fieldsDiv"+workflowId).append(dateTimeField);
                  break;
            case 'Check Box':
                  var checkBoxOptions=response[i]['options'];
                  ...
                  var checkBoxField=checkBoxFields(fieldLabel,fieldName,checkBoxOptions,fieldValue,commentvalue,disabled,"",response[i]['hide_field']);
                  $("#fieldsDiv"+workflowId).append(checkBoxField);
                  break;
            case 'Multivalue':
               var value= moment(response[i]['fieldValue'],'YYYY-MM-DD',true);
         ...
         var formula_dependent=response[i]['formula_dependent'];
         var fieldValue=response[i]['fieldValue'];
         var hiddenFormulaField=hiddenFormulaFields(response[i]['field_name'],formula,formula_dependent,fieldValue);
         $("#fieldsDiv"+workflowId).append(hiddenFormulaField);         
      } else if(response[i]['field_type']=='Formula') {
...
function submitAfterValidate() {
   ...
   var record_id=$("#recordId").val();
   var workflowId=$('#workflowId').val();
   var myform = $('#workflowForm');
   var disabled = myform.find(':input:disabled').removeAttr('disabled');
   var formDataField = myform.serializeArray();
   ...
   $.ajax({
      url: '/wp/updateAndSaveWorkflowData/'+tracker_id+'/'+form_id+'/'+dashboardId+'/'+asId+'/'+record_id+'/'+workflowId, 
      type: 'post',
      dataType: 'json',
      data: formData,
      processData: false,
      ...
      success: function(data) {
         ...
         if (record_id == 0 && data.recordId != 0) {
             record_id=data.recordId;
             window.location.href='/wp/editrecord/'+tracker_id+'/'+form_id+'/'+dashboardId+'/'+asId+'/'+record_id+'?filter='+filter;
         }
         $("#submitButton"+workflowId).attr("disabled", false);
      },
   ...
   $('.formuladate').each(function () {
      ...
      if (typeof formulaDepDate !== 'undefined' && formulavalDate !== 'null' && formulavalDate != '') {
         ...
         if(formulaDepDateArr.includes(id)){
            ...
            if(dateFieldValue == 'Invalid date' || dateFieldValue == 'manual'){
               $('#'+formulaDateId).val("");
         ...
         if(formulaDepArr.includes(id)){
            ...
            $.each(formulaDepArr, function (index, value_dep) {
               ...
               if(isNaN(depValue) || depValue ==''){ 
                  formulaExpression = formulaExpression.replace(new RegExp(formulaId, 'g'),$('#'+value_dep).val());
               } else {
                  formulaExpression = formulaExpression.replace(new RegExp('"'+formulaId+'"', 'g'),$('#'+value_dep).val());
               }
            }); 
