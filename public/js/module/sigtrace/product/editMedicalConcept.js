...
function editMedicalConcept(trackerId, medicalConceptId, $activeSubstanceId, formId) {
...
//    }
    ...
    if (count == 0) {
        ...
        var url = "/medicalconcept/saveEditMedicalConcept/"+trackerId+"/"+formId;
        $.post(url, data,function(respJson){
            ...
            if((responseCode == 1 || responseCode == 2) && (responseCode != null)){
                window.location.assign('/medicalconcept/medicalconcept_management/'+trackerId+'/'+formId+'/'+$activeSubstanceId);
            }
            else{
