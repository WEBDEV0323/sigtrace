...
$('#n_condtionfield').change(function(){
    ...
    if(field== null || field==""){
        ...
        //$('#n_condtionfield_first').html('<font color="#cc0000">This field is required. </font>');
    } else {
        $("#n_condition_first").html('');   
    }
});
...
function addNewTemplate(tracker_id,template_id) {
    ...
    if(ccmail!=="") {
        ...
        for(var i = 0;i < emails.length;i++) {
            ...
            if(!regex.test(emails[i])) {
                $("#cc_status").html('<span class="error">Please enter valid email Id.</span>');
                flag = false;
            }else{
                $("#cc_status").html('');
            }
    ...
    if (!valid || flag==false) {
        ...
        if(template_id>0){
            if(field_id==null){
                $("#sendTo_status").html('<span class="error">This field is required. </span>');
                return false;
            }
            if(ccmail!=null){
                var emails = ccmail.split(",");
...
$("#n_cc").change(function(){ 
    ...
    if(ccmail!=null){
        if(ccmail.indexOf(";") > -1){
            $("#cc_status").html('<span class="error">Semicolon(;) is not allowed. </span>');
            return false;
        }else{
            var emails = ccmail.split(","); 
            var regex = /^([\w-\.\#]+@([\w-]+\.)+[\w-]{2,4})?$/;
...
$("#n_name").change(function (){
    var re =  new RegExp(/^[a-zA-Z0-9.#_ ]*$/);
    var notificationName = $.trim($("#n_name").val());
    if (notificationName.length === 0) {
        $("#n_name_first").html('<span class="error">Notification name cannot be blank</span>');
        count++;
    } else {
            ...
            $.ajax({
                ...
                success: function (msg) {
                    if (JSON.parse(msg) == 'ok'){
                        $("#n_name_first").html('<span class="error">This Name is already exists in the system. Please try with a different Name.</span>');
                        count++;
                    } else {
                        $("#n_name_first").html('<span class="error"></span>');
                        (count!=0) ? count-- : "";
                    }
                }
            });
        } else {
            $("#n_name_first").html('<span class="error">Notification Name is not in proper format</span>');
            count++;
        }
    }
    if (count>0) {
