    ...
    if(form_id>0) {
        ...
        $.each(dateFields, function (i) {
            if (currentFieldVal == dateFields[i].field_name) {
                $("#dateFields").append("<option value=" + dateFields[i].field_name + " selected='selected'>" + dateFields[i].field_name + "</option>");
            } else { 
                $("#dateFields").append("<option value=" + dateFields[i].field_name + ">" + dateFields[i].field_name + "</option>");
            }
            });
...
function addConditionFields(i, fieldName='', operand='', value='') {
    ...
    if (operand == "<") {
        html+='<option value="<" selected="selected">&lt</option>';
    } else {
...
$("#formname").on("change", function(event) {
    ...
    $("#workflowFields").selectpicker('refresh');
    $("#fieldsForWorkflow").selectpicker('refresh');
    $("#notifyWhom").selectpicker('refresh').empty().trigger('change');
    $('.selectpicker').selectpicker('setStyle', 'btn-light', 'remove');
    $('.selectpicker').selectpicker('setStyle', 'dropdown-select', 'add'); 
});
            ...
            $.ajax({
                ...
                success: function (msg) {
                    if (JSON.parse(msg) == 'ok'){ 
                        $("#NotificationName_err").html('<label class="error">This Name is already exists in the system. Please try with a different Name.</label>');
                        count++;
                    } else {
                        $("#NotificationName_err").html('<label class="error"></label>');
                        (count!=0) ? count-- : "";
                    }
                }
            });
        } else {
            $("#NotificationName_err").html('<label class="error">Reminder Name is not in proper format</label>');
            count++;
        }
    }
    if (count>0) {
...
$("#subject").change(function (){ 
    $('#savereminder').prop('disabled',false);
     ...
     if (subject.length === 0) {
        $("#subject_err").html('<label class="error">Subject cannot be blank</label>');
        count++;
    } else {
        if (re.test(subject)){
            ...
            $("#subject_err").html('<label class="error">Subject is not in proper format</label>');
            count++;
...
function addNewReminder(tracker_id,template_id) {
    ...
    if(ccmail!=="") {
        ...
        for(var i = 0;i < emails.length;i++) {
            ...
            if(!regex.test(emails[i])) {
                $("#CCToWhom_err").html('<label class="error">Please enter valid email Id.</label>');
                flag = false;
            }
        }
    }
    ...
    if(!valid || flag==false) {
        ...
        if(template_id>0){
            if(field_id==null){
                $("#notifyWhom_err").html('<label class="error"><b>This field is required.</b> </label>');
                return false;
            }
            if(n_cond==null){
                $("#NotificationWhen_error").html('<label class="error"><b>This field is required.</b> </label>');
                return false;
            }
            if(ccmail!=null){
                var emails = ccmail.split(",");
            ...
            if(ccmail!=null){
                ...
                for(var i = 0;i < emails.length;i++) {
                    emails[i]=emails[i].trim();
                    if(!regex.test(emails[i])) {
                        $("#CCToWhom_err").html('<label class="error"><b>Please enter valid email Id.</b> </label>');
                        return false;
                    }
                }
            }
...
function reasonforchange(){
    ...
    $("#CCToWhom").change(function(){ 
        ...
        if(ccmail!=null){
            if(ccmail.indexOf(";") > -1){
                $("#CCToWhom_err").html('<label class="error"><strong>Semicolon(;) is not allowed.</strong> </label>');
                return false;
            }else{
                var emails = ccmail.split(","); 
                var regex = /^([\w-\.\#]+@([\w-]+\.)+[\w-]{2,4})?$/;
